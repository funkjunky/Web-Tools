...
{return this.each(function(){$this=$(this).data("multiSelection");var newOptions=$this.options;newOptions["data"]=arr;$(this).multiSelection("destroy");alert(newOptions);$(this).multiSelection(newOptions);});}},init:function(){var $this=$(this).data("multiSelection");var selection=$(this);if(count($this["data"])<=0)
...
selection.after($this.cbContainer);selection.bind("change.multiSelection",selectionOnChange);$this.autocomplete=new portableAutoComplete({arr:$.makeArray($this["data"]),css:{position:"absolute",top:selection.offset().top+selection.height()+6+"px",left:selection.offset().left+"px"},selectionCallback:$this.selectionCallback});$(document.body).append($this.autocomplete.container);selection.bind("focus.multiSelection",function(){$(this).keyup();$this.autocomplete.container.css({top:selection.offset().top+selection.height()+6+"px",left:selection.offset().left+"px"});$this.autocomplete.container.show();});$this.autocomplete.container.bind("mousemove.portableAutoComplete",function(){$this.onMenu=true;});$this.autocomplete.container.bind("mouseout.portableAutoComplete",function(){$this.onMenu=false;});selection.bind("blur.multiSelection",function(){if(!$this.onMenu){$this.autocomplete.container.hide();$this.autocomplete.index=false;}});selection.bind("keydown.multiSelection",keydown);selection.bind("keyup.multiSelection",keyup);},getItemByCaret:function(jdom){var startComma=getFirstCharBeforeCaret(jdom,",");if(startComma!=0)++startComma;var endComma=getFirstCharAfterCaret(jdom,",");var currentItem=$.trim(jdom.val().substring(startComma,endComma));return currentItem;},disable:function(){this.attr("readonly",true);this.each(function(){var $this=$(this).data("multiSelection");$this.cbButton.attr("disabled",true);for(var i in $this.cbs)
